...
class FY4a3kDataset(Dataset):

    def __init__(self, X, Y, use_augment=False, data_name='fy4a_3k'):
        super(FY4a3kDataset, self).__init__()
...
def load_data(batch_size, val_batch_size, data_root, num_workers=1,
    ...
    traindata = np.load(os.path.join(data_root, 'FY4A_3k/train.npy'))
    X_train = traindata[:,:pre_seq_length,:,:,:]
    Y_train = traindata[:,pre_seq_length:,:,:,:]

    testdata = np.load(os.path.join(data_root, 'FY4A_3k/test.npy'))
    ...
    train_set = FY4a3kDataset(X=X_train, Y=Y_train, use_augment=False)
    test_set = FY4a3kDataset(X=X_test, Y=Y_test, use_augment=False)
