...
class Cloudtc_meteo(Dataset):
    ...
    def __init__(self, data_root, split, image_size=256,
                 pre_seq_length=4, aft_seq_length=4, data_name='cloudtc_meteo'):
        super(Cloudtc_meteo, self).__init__()
        ...
        self.time_list = pd.timedelta_range(start='07:42:00', end='15:42:00', freq='30min')
    ...
    def __getitem__(self, idx):
        ...
        for i in range(self.seq_length):
            # import pdb;
            # pdb.set_trace()
            img_path = os.path.join(self.data_root, 'meteo_png', self.date_list[date_idx]+self.time_list[time_idx + i] + '.png')
...
def load_data(batch_size, val_batch_size, data_root, num_workers=0, data_name='cloudtc_meteo',
              pre_seq_length=4, aft_seq_length=4, in_shape=[4, 3, 256, 256],
              distributed=False, use_augment=False, use_prefetcher=False, drop_last=False):
    if data_name == 'cloudtc_meteo':
        data_root = os.path.join(data_root, 'meteo')
    else:
    ...
    train_set = Cloudtc_meteo(data_root, 'train', image_size,
                        pre_seq_length=pre_seq_length, aft_seq_length=aft_seq_length)
    val_set = Cloudtc_meteo(data_root, 'test', image_size,
                        pre_seq_length=pre_seq_length, aft_seq_length=aft_seq_length)
    test_set = Cloudtc_meteo(data_root, 'test', image_size,
                        pre_seq_length=pre_seq_length, aft_seq_length=aft_seq_length)
...
if __name__ == '__main__':
    dataset = Cloudtc_meteo("/nas2/data/users/xmq/Prediction/OpenSTL-OpenSTL-Lightning/data/meteo/", 'test')
    import cv2
