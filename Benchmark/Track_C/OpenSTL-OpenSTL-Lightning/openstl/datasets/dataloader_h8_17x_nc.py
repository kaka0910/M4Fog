...
class H817xncDataset(Dataset):

    def __init__(self, X, Y, use_augment=False, data_name='h8_17x'):
        super(H817xncDataset, self).__init__()
...
def load_data(batch_size, val_batch_size, data_root, num_workers=1,
    ...
    traindata = np.load(os.path.join(data_root, 'H8_17x/train_nc_640.npy'))
    X_train = traindata[:,:pre_seq_length,:,:,:]  #[:4]=img
    Y_train = traindata[:,-pre_seq_length:,:,:,:] #[-4:]=label

    testdata = np.load(os.path.join(data_root, 'H8_17x/val_nc_400.npy'))
    ...
    train_set = H817xncDataset(X=X_train, Y=Y_train, use_augment=False)
    test_set = H817xncDataset(X=X_test, Y=Y_test, use_augment=False)
