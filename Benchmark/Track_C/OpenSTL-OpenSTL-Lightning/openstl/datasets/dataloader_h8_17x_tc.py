...
class H817xtcDataset(Dataset):

    def __init__(self, X, Y, use_augment=False, data_name='h8_17x'):
        super(H817xtcDataset, self).__init__()
...
def load_data(batch_size, val_batch_size, data_root, num_workers=1,
    ...
    # Y_train = traindata[:,-pre_seq_length:,0:1,:,:] #[-4:]=label [640, 4, 1, 256, 256]
    ...
    # Y_test = testdata[:,-pre_seq_length:,0:1,:,:]
    ...
    train_set = H817xtcDataset(X=X_train, Y=Y_train, use_augment=False)
    test_set = H817xtcDataset(X=X_test, Y=Y_test, use_augment=False)
