...
def fog_metrics(pred, true, threshold):
    """
    calcaulate t, p, hits, fas, misses
    Inputs:
    pred: [N, T, C, L, L]
    ...
    只计算mask也就是后三个通道的iou
    """
    pred_fog = pred[:,:,-3:,:,:].transpose(1, 0, 2, 3, 4)
    true_fog = true[:,:,-3:,:,:].transpose(1, 0, 2, 3, 4)
    hits, fas, misses = [], [], []
    for i in range(pred_fog.shape[0]):
        t, p = _threshold(pred_fog[i], true_fog[i], threshold)
        hits.append(np.sum(t * p))
        fas.append(np.sum((1 - t) * p))
        misses.append(np.sum(t * (1 - p)))
    return np.array(hits), np.array(fas), np.array(misses)
...
def metric(pred, true, mean=None, std=None, metrics=['mae', 'mse'],
    ...
    eval_res = {}
    eval_log = ""
    allowed_metrics = ['mae', 'mse', 'rmse', 'ssim', 'psnr', 'snr', 'lpips', 'pod', 'sucr', 'csi', 'csi_fog']
    ...
    if 'csi_fog' in metrics:
        hits, fas, misses = fog_metrics(pred, true, threshold=0.5)
        eval_res['csi_fog'] = CSI(hits, fas, misses)
