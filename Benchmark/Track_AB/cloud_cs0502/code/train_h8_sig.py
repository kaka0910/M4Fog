...
from encoding.utils import (AverageMeter, LR_Scheduler,
                            intersectionAndUnionGPU, intersectionAndUnionGPU_sig, save_checkpoint)
...
def main_worker(gpu, ngpus_per_node, argss):
    ...
    val_data = get_segmentation_dataset(args.dataset, root=args.data_root, split='val')
        ...
        # if epoch >= int(args.epochs * 0.3) and (epoch+1) % 2 == 0: ##epochs*0.2
            ...
            if main_process() and mIoU_val >= best_mIoU:
                ...
                torch.save({'epoch': epoch_log, 
                        ...
                        filename)
    if main_process():
        filename = args.model_savefolder + 'last_model_0305.pth'
        torch.save({'epoch': epoch_log, 
...
def train(train_loader, model, scaler, optimizer, epoch, scheduler, nclass,currentSteps,wandbHandle=None):
    ...
    for i, (input, target) in enumerate(train_loader):
        ...
        # import pdb;pdb.set_trace()
        intersection, union, target = intersectionAndUnionGPU_sig(output, target.squeeze(dim=1), 2, 10)
...
def validate(val_loader, model, criterion, nclass,epoch):
    ...
    for i, (input, target) in enumerate(val_loader):
        ...
        # print(output.shape)
        loss = criterion(output.unsqueeze(dim=1),target)
        ...
        if(i==0):
            Task.current_task().get_logger().report_image("val", "input", iteration=epoch, image=input.cpu().numpy().astype(np.uint8)[0,[2,3,13]].transpose([1,2,0])[:,:,[2,1,0]])
            Task.current_task().get_logger().report_image("val", "target", iteration=epoch, image=target.cpu().numpy().astype(np.uint8)[0]*255)
            Task.current_task().get_logger().report_image("val", "output", iteration=epoch, image=output.cpu().numpy().astype(np.uint8)[0]*255)
...
if __name__ == '__main__':
    args = Options().parse()
    # task = Task.init(project_name='seafog', task_name='aspp_unet_0309', auto_connect_arg_parser={'rank': False})  # noqa: F841
