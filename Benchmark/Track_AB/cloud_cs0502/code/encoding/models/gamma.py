...
class Graph_layer(nn.Module):
    def __init__(self, num_state, num_node, num_class):
        ...
        self.gamma_vis = Parameter(torch.zeros(num_node))
        self.gamma_word = Parameter(torch.zeros(num_class))

    def forward(self, inp, V):
        new_inp = self.word_gcn(inp)
        new_V = self.vis_gcn(V)
        class_node = self.v2s_transfer(new_V)
        vis_node = self.s2v_transfer(new_inp)

        class_node = self.gamma_word * new_inp + class_node
