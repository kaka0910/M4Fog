...
def get_s2unet_v4(dataset='cloud', **kwargs):
    # infer number of classes
    from ..datasets import datasets
    model = S2UNet_v4(num_classes=datasets[dataset.lower()].NUM_CLASS, **kwargs)
    return model

class S2UNet_v4(nn.Module):
    def __init__(
        self,
        image_size=256,
        patch_size=4,
        ...
        super(S2UNet_v4, self).__init__()
        self.base_size = 256
        self.crop_size = 256
        self.nclass = num_classes
        
        self.embed = nn.Sequential(
            nn.Conv2d(in_channels, embed_dim, kernel_size=1, stride=1),
            nn.ReLU(inplace=True),
            nn.BatchNorm2d(embed_dim)
        )
        self.in_block = self.stage(dim_in=embed_dim, depth=4, expansion_factor=4, dropout = 0.5)
        self.down_4x = nn.Sequential(
            FirstPatchMerging(dim=embed_dim, norm_layer=nn.LayerNorm),
            FirstPatchMerging(dim=embed_dim, norm_layer=nn.LayerNorm)
        )
        ...
        self.out_conv0 = ConcatBackDim(dim=embed_dim)
    ...
    def forward(self, x, y=None):
        ...
        out = self.out_conv0(self.up_4x(out), x0)
