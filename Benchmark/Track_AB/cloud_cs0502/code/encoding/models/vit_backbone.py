...
class Attention(nn.Module):
    ...
    def forward(self, x, H, W):      # H, W 没有被用到, 只是为了和 WindowAttention 保持一致
        ...
        qkv = self.qkv(x).contiguous().reshape(B, N, 3, self.num_heads, -1).permute(2, 0, 3, 1, 4)
...
def calc_rel_pos_spatial(
    ...
    r_q = q[:, :, sp_idx:].contiguous().reshape(B, n_head, q_h, q_w, dim)
    rel_h = torch.einsum("byhwc,hkc->byhwk", r_q, Rh)
    rel_w = torch.einsum("byhwc,wkc->byhwk", r_q, Rw)

    attn[:, :, sp_idx:, sp_idx:] = (
        attn[:, :, sp_idx:, sp_idx:].contiguous().view(B, -1, q_h, q_w, k_h, k_w)
        + rel_h[:, :, :, :, :, None]
        + rel_w[:, :, :, :, None, :]
    ).contiguous().view(B, -1, q_h * q_w, k_h * k_w)
...
class WindowAttention(nn.Module):
    ...
    def forward(self, x, H, W):
        ...
        """
        B_, N, C = x.shape
        x = x.contiguous().reshape(B_, H, W, C)
        pad_l = pad_t = 0
        ...
        x = x.contiguous().view(-1, self.window_size[1] * self.window_size[0], C)  # nW*B, window_size*window_size, C
        B_w = x.shape[0]
        N_w = x.shape[1]
        qkv = self.qkv(x).contiguous().reshape(B_w, N_w, 3, self.num_heads, C // self.num_heads).permute(2, 0, 3, 1, 4)
        q, k, v = qkv[0], qkv[1], qkv[2]  # make torchscript happy (cannot use tensor as tuple)
        ...
        x = (attn @ v).transpose(1, 2).contiguous().reshape(B_w, N_w, C)
        x = self.proj(x)
        x = self.proj_drop(x)

        x = x.contiguous().view(-1, self.window_size[1], self.window_size[0], C)
        x = window_reverse(x, self.window_size[0], Hp, Wp)  # B H' W' C

        if pad_r > 0 or pad_b > 0:
            x = x[:, :H, :W, :].contiguous()

        x = x.contiguous().view(B_, H * W, C)
...
class Block(nn.Module):
    def __init__(self, dim, num_heads, mlp_ratio=4., qkv_bias=False, qk_scale=None, drop=0., attn_drop=0.,
        ...
        self.filter = GlobalFilter(dim, h=16, w=9)


    def forward(self, x, H, W):
        if self.gamma_1 is None:
            x = x + self.drop_path(self.attn(self.filter(self.norm1(x)), H, W))
...
class PropogationBasicBlock(nn.Module):
    ...
    def forward(self, x, H, W):
        ...
        x = x.contiguous().reshape(batch_size, H, W, channel_cnt)
        ...
        out = out.contiguous().reshape(batch_size, H*W, channel_cnt)
...
class ViTBackbone(nn.Module):
    ...
    """
    def __init__(self, img_size=256, patch_size=16, in_chans=16, num_classes=80, embed_dim=768, depth=12,
        ...
        if self.patch_size==16:
            ...
            self.fpn1 = nn.Sequential(
                ...
                nn.GELU(),
                nn.ConvTranspose2d(embed_dim, embed_dim, kernel_size=2, stride=2),
                Conv1x1LN(embed_dim, dims[0])    # 768 -> 64
            )
            self.fpn2 = nn.Sequential(
                nn.ConvTranspose2d(embed_dim, embed_dim, kernel_size=2, stride=2),
                Conv1x1LN(embed_dim, dims[1])    # 768 -> 128
            )
            self.fpn3 = nn.Sequential(
                nn.Identity(),
                Conv1x1LN(embed_dim, dims[2])    # 768 -> 256
            )
            self.fpn4 = nn.Sequential(
                nn.MaxPool2d(kernel_size=2, stride=2),
                Conv1x1LN(embed_dim, dims[3])    # 768 -> 512
            )
    ...
    def load_pytorch_pretrained_backbone(self, pretrained_backbone_path):
        ...
        pretrained_model = torch.load(pretrained_backbone_path, map_location='cuda')['state_dict']
        state_dict = {}

        # ################### interpolate pos_embed ###################
        pos_embed_checkpoint = pretrained_model['vit_backbone.pos_embed']
        ...
        if orig_size != new_size:
            ...
            pos_tokens = pos_tokens.contiguous().reshape(-1, orig_size, orig_size, embedding_size).permute(0, 3, 1, 2)
            pos_tokens = torch.nn.functional.interpolate(
                pos_tokens, size=(new_size, new_size), mode='bicubic', align_corners=False)
            pos_tokens = pos_tokens.permute(0, 2, 3, 1).flatten(1, 2)
            new_pos_embed = torch.cat((extra_tokens, pos_tokens), dim=1)
            state_dict['vit_backbone.pos_embed'] = new_pos_embed
        # ##############################################################

        for key, value in pretrained_model.items():
            if key.startswith('vit_backbone.patch_embed') or key.startswith('vit_backbone.norm') or key.startswith('vit_backbone.blocks'):
    ...
    def forward_features(self, x):
        ...
        x = self.norm(x)
        xp = x.permute(0, 2, 1).contiguous().reshape(B, -1, Hp, Wp)
...
def resize_pos_embed(posemb, posemb_new, num_tokens=1, gs_new=()):
    ...
    posemb_grid = posemb_grid.contiguous().reshape(1, gs_old, gs_old, -1).permute(0, 3, 1, 2)
...
def _load_weights(model, checkpoint_path: str, prefix: str = ''):
    ...
    for i, block in enumerate(model.blocks.children()):
        ...
        block.attn.qkv.bias.copy_(torch.cat([
            _n2p(w[f'{mha_prefix}{n}/bias'], t=False).contiguous().reshape(-1) for n in ('query', 'key', 'value')]))
