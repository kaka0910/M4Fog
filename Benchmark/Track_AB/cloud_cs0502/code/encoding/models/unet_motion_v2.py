...
def get_unet_motion_v2(dataset='cloud', **kwargs):
    # infer number of classes
    from ..datasets import datasets
    model = Unet_motion_v2(in_channels=17, num_classes=2, **kwargs)
    return model
...
class sobel(nn.Module):
    ...
    def get_motion(self, motion_np):
        motion_layer = motion_np[:,-1,:,:] # 第一个通道存的是motion motion_np.shape = (7,1024,1024)
        motion_layer[motion_layer<1] = 0
        motion_layer = motion_layer.unsqueeze(1)
    ...
    def forward(self):
        ...
        if self.cal_motion == True:
            ...
            motion_tensor = self.get_motion(motion_np)
            ...
            if self.cal_sobel == True:
                ...
                # rgb_red = torch.cat((og_im, motion_tensor), 1)
                # rgb_red_conved = torch.cat((rgb_red, motion_conv), 1)
                rgb_red_conved = torch.cat((og_im, motion_conv), 1)
        ...
        final = self.gamma * (sigmoid_output * og_im) + (1 - self.gamma)*og_im
        return final
    
class Unet_motion_v2(nn.Module):
    def __init__(
        self, 
        in_channels=14,
        num_classes=2,
        ...
        super(Unet_motion_v2, self).__init__()
        ...
        # self.conv_dist = nn.Conv2d(1, 4, 3, 1, 1)
    ...
    def forward(self, x, y=None):
        # import pdb;
        # pdb.set_trace()
        img = torch.concatenate([x[:,:16,:,:],x[:,-1:,:,:]], dim=1).clone()
        # conv_dist = self.conv_dist(dist)
        att_map = sobel(img, n_channels=17, cal_motion=True, cal_sobel=False).cuda()
        new_dist = att_map.forward().cuda()
        
        conv1_out = self.down1(new_dist)
