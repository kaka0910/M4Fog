...
def get_unet_temporal(dataset='cloud', **kwargs):
    # infer number of classes
    from ..datasets import datasets
    model = Unet_temporal(in_channels=32, num_classes=2, **kwargs)
    return model
...
class Unet_temporal(nn.Module):
    ...
    def forward(self, x, y=None):
        ...
        out_1 = self.center(self.max_pool(conv4_out_1))
        
        conv1_out_2 = self.down1(x2)
        conv2_out_2 = self.down2(self.max_pool(conv1_out_2))
        conv3_out_2 = self.down3(self.max_pool(conv2_out_2))
        conv4_out_2 = self.down4(self.max_pool(conv3_out_2))
        out_2 = self.center(self.max_pool(conv4_out_2))
        
        out = out_1+out_2

        out = self.up4(torch.cat((self.trans4(out), conv4_out_1), 1))
        out = self.up3(torch.cat((self.trans3(out), conv3_out_1), 1))
        out = self.up2(torch.cat((self.trans2(out), conv2_out_1), 1))
        out = self.up1(torch.cat((self.trans1(out), conv1_out_1), 1))
