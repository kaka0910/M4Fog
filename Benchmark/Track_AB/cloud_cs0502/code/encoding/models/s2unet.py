...
def get_s2unet(dataset='cloud', **kwargs):
    # infer number of classes
    from ..datasets import datasets
    model = S2UNet(num_classes=datasets[dataset.lower()].NUM_CLASS, **kwargs)
    return model
...
class S2UNet(nn.Module):
    ...
    def forward(self, x, y=None):
        x = self.patch_embed(x) # B,Ph,Pw,48
