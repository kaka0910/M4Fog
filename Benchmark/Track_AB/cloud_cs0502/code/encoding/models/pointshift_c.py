...
# print('------------------------------------------------------------------')
def get_s2unet_v7c(dataset='cloud', **kwargs):
    # infer number of classes
    from ..datasets import datasets
    model = S2UNet_v7c(num_classes=datasets[dataset.lower()].NUM_CLASS, **kwargs)
    return model

class S2UNet_v7c(nn.Module):
    def __init__(
        self,
        image_size=256,
        in_channels=16,
        ...
        super(S2UNet_v7c, self).__init__()
        self.base_size = 256
        self.crop_size = 256
        self.nclass = num_classes
        
        self.embed = nn.Conv2d(in_channels, embed_dim, kernel_size=1, stride=1)
        self.block = self.stage(dim_in=embed_dim, depth=1, expansion_factor=4, dropout = 0.) # 0.5
        self.down1 = self.down_stage(dim_in=embed_dim, depth=2, expansion_factor=4, dropout = 0.04)
        self.down2 = self.down_stage(dim_in=embed_dim*2, depth=2, expansion_factor=4, dropout = 0.08)
        self.down3 = self.down_stage(dim_in=embed_dim*4, depth=2, expansion_factor=4, dropout = 0.12)
        self.down4 = self.down_stage(dim_in=embed_dim*8, depth=6, expansion_factor=4, dropout = 0.16)
        self.down5 = self.down_stage(dim_in=embed_dim*16, depth=2, expansion_factor=4, dropout = 0.2)

        self.up5 = self.up_stage(dim_in=embed_dim*32, depth=2, expansion_factor=4, dropout = 0.16)
        self.up4 = self.up_stage(dim_in=embed_dim*16, depth=2, expansion_factor=4, dropout = 0.12)
        self.up3 = self.up_stage(dim_in=embed_dim*8, depth=2, expansion_factor=4, dropout = 0.08)
        self.up2 = self.up_stage(dim_in=embed_dim*4, depth=2, expansion_factor=4, dropout = 0.04)
        self.up1 = self.up_stage(dim_in=embed_dim*2, depth=2, expansion_factor=4, dropout = 0.)

        self.out_conv5 = ConcatBackDim(dim=embed_dim*16)
        self.out_conv4 = ConcatBackDim(dim=embed_dim*8)
...
class S2Attention(nn.Module):
    def __init__(self, channels=512):
        ...
        self.mlp2 = nn.Linear(channels * 3, channels)
