...
def get_s2unet_v3(dataset='cloud', **kwargs):
    # infer number of classes
    from ..datasets import datasets
    model = S2UNet_v3(num_classes=datasets[dataset.lower()].NUM_CLASS, **kwargs)
    return model

class S2UNet_v3(nn.Module):
    def __init__(
        self,
        image_size=256,
        in_channels=16,
        ...
        self.embed = nn.Conv2d(in_channels, embed_dim[0], 1, bias=False),
        self.block = self.stage(dim_in=embed_dim[0], depth=depth[0], expansion_factor=4, dropout = 0.5)
        self.down1 = self.down_stage(dim_in=embed_dim[0], depth=depth[1], expansion_factor=4, dropout = 0.5)
        self.down2 = self.down_stage(dim_in=embed_dim[1], depth=depth[2], expansion_factor=4, dropout = 0.5)
        self.down3 = self.down_stage(dim_in=embed_dim[2], depth=depth[3], expansion_factor=4, dropout = 0.5)
        self.down4 = self.down_stage(dim_in=embed_dim[3], depth=depth[4], expansion_factor=4, dropout = 0.5)   

        self.up4 = self.up_stage(dim_in=embed_dim[3]*2, depth=depth[-4], expansion_factor=4, dropout = 0.5)
        self.up3 = self.up_stage(dim_in=embed_dim[2]*2, depth=depth[-3], expansion_factor=4, dropout = 0.5)
        self.up2 = self.up_stage(dim_in=embed_dim[1]*2, depth=depth[-2], expansion_factor=4, dropout = 0.5)
        self.up1 = self.up_stage(dim_in=embed_dim[0]*2, depth=depth[-1], expansion_factor=4, dropout = 0.5)
    ...
    def stage(self, dim_in, depth=4, expansion_factor=4, dropout = 0.):
        return S2Block(dim_in, depth, expansion_factor, dropout = 0.)

    def down_stage(self, dim_in, depth=4, expansion_factor=4, dropout = 0.):
        return nn.Sequential(
            PatchMerging(dim=dim_in, norm_layer=nn.LayerNorm),
            S2Block(dim_in*2, depth, expansion_factor, dropout = 0.)
        )

    def up_stage(self, dim_in, depth=4, expansion_factor=4, dropout = 0.):
        return nn.Sequential(
    ...
    def forward(self, x, y=None):
        ...
        x1 = self.block(embedding)      # x
        x2 = self.down1(x1)     # x/2
        x3 = self.down2(x2)     # x/4
        x4 = self.down3(x3)     # x/8

        # bottleneck
        center = self.down4(x4) # x/16
