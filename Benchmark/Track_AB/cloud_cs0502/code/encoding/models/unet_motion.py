...
def get_unet_motion(dataset='cloud', **kwargs):
    # infer number of classes
    from ..datasets import datasets
    model = Unet_motion(in_channels=18, num_classes=2, **kwargs)
    return model

class Unet_motion(nn.Module):
    def __init__(
        self, 
        in_channels=14,
        num_classes=2,
        ...
        super(Unet_motion, self).__init__()
    ...
    def forward(self, x, y=None):
        x = torch.concatenate([x[:,:16,:,:],x[:,-1:,:,:]], dim=1)
