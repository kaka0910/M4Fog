...
from .vit_backbone_v2 import ViTBackbone
from .pointshift import *


def get_Dlink_vit_v2(dataset='cloud', **kwargs):
    # infer number of classes
    from ..datasets import datasets
    model = Dlink_vit_v2(patch_size=16, num_classes=datasets[dataset.lower()].NUM_CLASS, **kwargs)
    return model


class Dlink_vit_v2(nn.Module):
    params = {
    ...
    def __init__(
        ...
        super(Dlink_vit_v2, self).__init__()
        self.base_size = 256
        self.crop_size = 256
        self.nclass = num_classes
        self.patch_size = patch_size
        
        # self.conv1=nn.Sequential(nn.Conv2d(in_channels=16, out_channels=8, kernel_size=1, stride=1, padding=0),
        # nn.Conv2d(in_channels=8, out_channels=4, kernel_size=1, stride=1, padding=0),
        # nn.Conv2d(in_channels=4, out_channels=3, kernel_size=1, stride=1, padding=0),
        # ) 
