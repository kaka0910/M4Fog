...
class SAM(nn.Module):
    def __init__(self, in_channels, reduct=2):
...
class CCAtt(nn.Module):
    def __init__(self, in_channels, recurrence=2):
        super(CCAtt, self).__init__()
...
class JPU(nn.Module):
    def __init__(self, in_channels, norm_layer=nn.BatchNorm2d, criterion=None):
        super(JPU, self).__init__()
        width = in_channels // 4
        self.dilation1 = nn.Sequential(SeparableConv2d(in_channels, width, kernel_size=1, padding=0, dilation=1, bias=False), # utilize kernel_size=1
                                       norm_layer(width),
                                       nn.LeakyReLU(inplace=True),
                                    #    SAM(width, reduct=2)
                                       )
        self.spg1 = SPG_(width, 10, norm_layer, criterion)
        self.dilation2 = nn.Sequential(SeparableConv2d(in_channels, width, kernel_size=3, padding=2, dilation=2, bias=False),
                                       norm_layer(width),
                                       nn.LeakyReLU(inplace=True),
                                    #    SAM(width, reduct=2)
                                       )
        self.spg2 = SPG_(width, 10, norm_layer, criterion)
        self.dilation3 = nn.Sequential(SeparableConv2d(in_channels, width, kernel_size=3, padding=4, dilation=4, bias=False),
                                       norm_layer(width),
                                       nn.LeakyReLU(inplace=True),
                                    #    SAM(width, reduct=2)
                                       )
        self.spg3 = SPG_(width, 10, norm_layer, criterion)
        self.dilation4 = nn.Sequential(SeparableConv2d(in_channels, width, kernel_size=3, padding=8, dilation=8, bias=False),
                                       norm_layer(width),
                                       nn.LeakyReLU(inplace=True),
                                    #    SAM(width, reduct=2)
                                       )
...
class VECNet(nn.Module):
    ...
    def __init__(self, nclass, criterion_seg=None, criterion_aux=None, aux=False, norm_layer=nn.BatchNorm2d):
        ...
        self.aux = aux
        self.stage_num = [2, 3, 3, 3, 2, 2] # [2, 2, 2, 2]
        
        # Input layer
        self.input = nn.Sequential(
            nn.Conv2d(16, self.BASE_WIDTH, 1, padding=0, bias=False),
            norm_layer(self.BASE_WIDTH),
            # nn.ReLU(inplace=True),
            nn.LeakyReLU(inplace=True),
            nn.Conv2d(self.BASE_WIDTH, self.BASE_WIDTH * 2, 1, padding=0, bias=False),
        ...
        # Stage - 1
        self.layer1 = self._make_layer(BasicBlock, self.BASE_WIDTH * 4, self.BASE_WIDTH * 4, self.stage_num[0], False)
        # Stage - 2
        self.layer2 = self._make_layer(BasicBlock, self.BASE_WIDTH * 4, self.BASE_WIDTH * 4, self.stage_num[1], False)
        ...
        # Stage - 3
        self.layer3 = self._make_layer(BasicBlock, self.BASE_WIDTH * 8, self.BASE_WIDTH * 8, self.stage_num[2], False)
        # Stage - 4
        self.layer4 = self._make_layer(BasicBlock, self.BASE_WIDTH * 8, self.BASE_WIDTH * 8, self.stage_num[3], False)
        ...
        # Decoder2 
        self.layer5 = self._make_layer(BasicBlock, self.BASE_WIDTH * 8, self.BASE_WIDTH * 4, self.stage_num[4], True)
        # Decoder1
        self.layer6 = self._make_layer(BasicBlock, self.BASE_WIDTH * 4, self.BASE_WIDTH * 4, self.stage_num[5], False)
    ...
    def forward(self, x, y=None):
        ...
        feat3 = self.down1(feat2)

        feat3 = self.layer3(feat3)
        feat4 = self.layer4(feat3)

        # if self.training: ##
        #     feat4, jpu_loss = self.attention(feat4, y) ##
        # else: ##
        #     feat4 = self.attention(feat4) ##

        feat5 = self.layer5(feat4)
        feat6 = self.layer6(feat5)

        feat6 = F.interpolate(feat6, scale_factor=2, mode='bilinear', align_corners=False)
        feat6 = self.up1(feat6) + feat2

        output = self.output(feat6)

        if self.training:
            main_loss = self.criterion_seg(output, y)
            if self.aux:
                auxout = self.auxlayer(feat3)
            ...
            return output.max(1)[1].detach(), main_loss
        else: 
            outputs = []
...
def get_vec(dataset='pascal_voc', **kwargs):
