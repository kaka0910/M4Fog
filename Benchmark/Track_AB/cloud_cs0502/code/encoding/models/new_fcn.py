...
class FCN(BaseNet):
    ...
    def forward(self, x,y=None):
        ...
        if self.training:
            loss = self.criterion_seg(outputs, y)
            # if self.aux:
            #     auxout = self.auxlayer(c3)
            #     auxout = upsample(auxout, imsize, **self._up_kwargs)
            #     aux_loss = self.criterion_aux(auxout, y)
            #     return outputs.max(1)[1].detach(), main_loss, aux_loss
            return outputs.max(1)[1].detach(), loss
        else:
...
def get_fcn(dataset='cloud', backbone='resnet50', pretrained=False,
