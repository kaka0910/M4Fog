...
def ResNet101(nInputChannels=16, os=16, pretrained=False):
    model = ResNet(nInputChannels, Bottleneck, [3, 4, 23, 3], os, pretrained=pretrained)
    return model
def ResNet18(nInputChannels=16, os=16, pretrained=False):
...
class DeepLabv3_plus(nn.Module):
    def __init__(self, criterion_seg, nInputChannels=16, n_classes=2, os=16, pretrained=False, _print=True):
        if _print:
    ...
    def forward(self, input, y=None):
        ...
        if self.training:
            loss = self.criterion_seg(x, y.long())
...
def get_deeplabv3p(dataset='h8',criterion_seg=None, **kwargs):
    # infer number of classes
    model = DeepLabv3_plus(criterion_seg, nInputChannels=14, n_classes=2, **kwargs)
    return model
