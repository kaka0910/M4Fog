...
class ContextPath(nn.Module):
    def __init__(self, pretrained=False, *args, **kwargs):
        ...
        self.resnet = timm.create_model('resnet18', features_only=True, output_stride=32,
                                        out_indices=(2, 3, 4), pretrained=None)
        self.resnet['conv1']=nn.Conv2d(14, 64, kernel_size=(7, 7), stride=(2, 2), padding=(3, 3), bias=False)
...
class SpatialPath(nn.Module):
    def __init__(self, *args, **kwargs):
        ...
        self.conv1 = ConvBNReLU(14, 64, ks=7, stride=2, padding=3)
...
def get_ABCNet(dataset='h8',criterion_seg=None, **kwargs):
    # infer number of classes
    model = ABCNet(criterion_seg=criterion_seg, **kwargs)
    return model

class ABCNet(nn.Module):
    def __init__(self, band=16, n_classes=2, pretrained=False, criterion_seg=None,):
    ...
    def init_weight(self):
        for ly in self.children():
            ...
            if isinstance(ly, nn.Linear):
                trunc_normal_(ly.weight, std=.02)
                if isinstance(ly, nn.Linear) and ly.bias is not None:
                    nn.init.constant_(ly.bias, 0)
...
if __name__ == "__main__":
    ...
    print()
    # net.train()
    # in_ten = torch.randn(2, 16, 1024, 1024)
    # out = net(in_ten)
    # print(out.shape)
    # print(out16.shape)
    # print(out32.shape)
