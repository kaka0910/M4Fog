...
def get_dlinkvit_h8_motion(dataset='h8', **kwargs):
    # infer number of classes
    # from ..datasets import datasets
    model = Dlink_vit_motion(patch_size=16, num_classes=2, **kwargs)
    return model

class Dlink_vit_motion(nn.Module):
    params = {
    ...
    def __init__(
        ...
        super(Dlink_vit_motion, self).__init__()
        self.base_size = 1024
        self.crop_size = 1024
        ...
        self.vit_backbone_dist = ViTBackbone(
            img_size=1024,
            patch_size=self.patch_size,
            in_chans=3,
            num_classes=2,
    ...
    def forward(self, x, y=None):
        ...
        # encoder
        e1_1, e2_1, e3_1, e4_1 = self.vit_backbone(x1)  #image
        e1_2, e2_2, e3_2, e4_2 = self.vit_backbone_dist(x2)
        ...
        # decoder
        d4 = self.decoder4(e4) + e3_1
        d3 = self.decoder3(d4) + e2_1
        d2 = self.decoder2(d3) + e1_1
        d1 = self.decoder1(d2)
