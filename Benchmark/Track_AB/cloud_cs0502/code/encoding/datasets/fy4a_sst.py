...
def default_loader(id, root, npz_root, split):
    ...
    cos = np.load('/groups/lmm2024/home/share/Sat_Pretrain_xmq/Data/SST_Data/land_mask.npy')
    cos = cos.reshape(1,1024,1024)
    

    return npy, mask, sst, cos
    
class FY4ASet_SST(data.Dataset):
    ...
    def __getitem__(self, index):
        id = self.ids[index]
        img, mask, sst, cos = self.loader(id, self.root, self.npz_root, self.split)
        img = torch.Tensor(img)
        img = (img-self.mean)/self.std
        
        sst = torch.FloatTensor(sst)
        cos = torch.FloatTensor(cos)
        
        # 合并img和sst
        combined_img = torch.cat((img, sst, cos),dim=0) #这里的shape应该是（17，1024，1024）
...
if __name__ == "__main__":
    a = FY4ASet_SST("/home/ssdk/mmengine/seafog_year_0412/","train")
    pass
